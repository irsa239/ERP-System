...
<body class="bg-gray-100 font-sans antialiased " class="..." x-data="{ results: [], query: '', show: false }">

<div class="flex h-screen">

    <!-- Sidebar -->
    <aside class="w-52 bg-blue-900 text-white flex-shrink-0 hidden md:block">
        <div class="p-6 text-2xl font-bold border-b border-blue-700">Admin Panel
            <div id="clock" class="text-white-100 font-semibold"></div>
        </div>
            
        <nav class="p-4 space-y-2 text-sm">
            <a href="/dashboard" class="block px-4 py-2 rounded hover:bg-blue-800">Dashboard</a>
            <a href="/employees" class="block px-4 py-2 rounded hover:bg-blue-800">Employees</a>
            <a href="/attendance" class="block px-4 py-2 rounded hover:bg-blue-800">Attendance</a>
            <a href="/leaves" class="block px-4 py-2 rounded hover:bg-blue-800">Leave Applications</a>
            <a href="/salary" class="block px-4 py-2 rounded hover:bg-blue-800">Salary Generation</a>
            <a href="/reports" class="block px-4 py-2 rounded hover:bg-blue-800">Reports</a>
            <a href="/settings" class="block px-4 py-2 rounded hover:bg-blue-800">Settings</a>
        </nav>
    </aside>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <nav class="bg-white shadow-md px-6 py-4 flex items-center justify-between sticky top-0 z-50">
            ...
            <div class="flex items-center gap-4">
                <img src="{{ asset('Images/logo.jpeg') }}" alt="Logo" class="h-12 w-12 rounded-full object-cover">

            <a href="{{ url('Images/logo.jpeg') }}" class="text-xl font-bold text-indigo-600 tracking-wide">
                     SmartStaff
                </a>
                ...
                <nav class="hidden lg:flex gap-4 text-md font-medium text-gray-600 space-x-2 ml-24">
                    ...
                    <a href="/teams" class="hover:text-indigo-600">Teams</a>
                </nav>
            </div>

          <!-- 🟡 Search Bar (very short length) -->
<div class="hidden lg:block">
    <div 
        class="relative w-[300px]" 
        x-data="{ results: [], query: '', show: false }" 
    ...
    >
        <input
            ...
            class="w-full border border-gray-300 rounded-full py-1 pl-3 pr-8 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
        ...
        <div 
            x-show="show && results.length > 0"
              x-cloak
            class="absolute bg-white border w-full mt-1 rounded shadow-lg max-h-64 overflow-y-auto z-50"
        >
            <template x-for="result in results" :key="result.url">
                <a :href="result.url" class="block px-4 py-2 hover:bg-gray-100 text-sm">
                    <span class="font-semibold text-indigo-600" x-text="result.type"></span>:
        ...
        <!-- ❌ No Results -->
        <div 
            x-show="show && results.length === 0 && query.length > 0"
             x-cloak
            class="absolute bg-white border w-full mt-1 rounded px-4 py-2 text-sm text-gray-500 z-50"
        >
            No results found.
        </div>
    </div>
...
<div class="relative" x-data="{ showNotifications: false }">
    <button @click="showNotifications = !showNotifications" class="relative focus:outline-none">
        <svg class="w-6 h-6 text-gray-600 hover:text-indigo-600" fill="none" stroke="currentColor" stroke-width="2"
             viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div x-show="showNotifications" @click.away="showNotifications = false"
         x-transition
         class="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded shadow-xl z-50 max-h-72 overflow-y-auto text-sm"
         x-cloak>
        ...
        @forelse($notifications->where('is_read', false) as $note)
            <div class="px-4 py-2 border-b hover:bg-gray-100">
                ...
                <form method="POST" action="{{ route('notifications.read', $note->id) }}">
                    @csrf
                    @method('PATCH')
                    <button class="text-indigo-600 text-xs mt-1 hover:underline" type="submit">Mark as read</button>
                </form>
            </div>
        @empty
        ...
        @endforelse
                ...
                <!-- Inbox -->
                <!-- Inbox (Chat) -->
<a href="{{ route('chat') }}" class="text-gray-600 hover:text-indigo-600">
    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2"
        ...
        <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h12a2 2 0 012 2z"/>
                ...
                <!-- Settings -->
                <a href="{{ route('help') }}" class="text-gray-600 hover:text-indigo-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.away="open = false" x-cloak x-transition
                        class="absolute right-0 mt-2 w-48 bg-white border rounded shadow-lg z-50 text-sm">
